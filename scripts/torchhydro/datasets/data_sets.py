...
class GPM_GFS_Dataset(Dataset):
    ...
    def _load_data(self):
        train_mode = self.is_tra_val_te == "train"
        if train_mode or self.is_tra_val_te == "valid":
            self.t_s_dict = wrap_t_s_dict(
                self.data_source, self.data_cfgs, self.is_tra_val_te
            )
        ...
        elif self.is_tra_val_te == "test":
            self.t_s_dict = wrap_t_s_dict(
                self.data_source, self.data_cfgs, self.is_tra_val_te
            )
            data_waterlevel_ds = self.data_source.read_test_waterlevel_xrdataset(
                self.t_s_dict["sites_id"],
                self.t_s_dict["t_final_range"],
                self.data_cfgs["target_cols"],
                self.data_cfgs["shap_name"],
                # "4150377B",
            )
            data_forcing_ds = self.data_source.read_test_gpm_xrdataset(
                self.t_s_dict["sites_id"],
                self.t_s_dict["t_final_range"],
                self.data_cfgs["relevant_cols"],
                self.data_cfgs["shap_name"],
